...
def locate_target_parts(target):
    ...
    try:
        ...
        embed = None
    if embed is None:
        for _n, m in target.named_modules():
...
def training_step_from_cache(
    assistant, target_embed, target_lm_head, batch, cfg: MTPLossConfig
):
    ...
    for k in range(K):
        ...
        hidden_k = hidden[:, :L, :]
        draft_logits, backbone_hidden = _assistant_step(
            assistant, target_embed, None, token_ids_k, hidden_k, shared_kv_states, None
        )
