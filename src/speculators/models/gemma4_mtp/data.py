"""Data pipeline for Gemma 4 MTP fine-tuning: conversations -> tensors.

Reads a target-regenerated JSONL (e.g. the output of a response-regeneration /
answer-regeneration pipeline): one JSON object per line with
...
The Gemma4 chat-template constants and the response-masking approach mirror the
verified gemma4 conversation parser (header/end-of-turn markers + a regex that
locates each assistant span), so this trainer masks tokens identically to the
data-generation line. Kept self-contained so the repo has no extra dependency.
...
# The sharded async cache engine lives in the target_cache module. Re-exported
# here so `from speculators.models.gemma4_mtp.data import CacheDataset, ...`
# keeps working.
from speculators.models.gemma4_mtp.target_cache import (  # noqa: E402,F401
    CacheDataset,
    collate_cache,
)
