...
Slow-mount problem: writing one .pt per sample straight to a network mount
...
def _tensor_bytes(tensor, dtype):
    ...
    raw = (
        ...
        else t.view(torch.uint16).view(torch.uint8)
    )
    if _HAS_NUMPY:
...
def build_sample_bytes(
    *, input_ids, loss_mask, last_hidden, kv_full_k, kv_full_v, kv_slide_k, kv_slide_v
):
    kv = {
        "kv_full_k": kv_full_k,
        "kv_full_v": kv_full_v,
        "kv_slide_k": kv_slide_k,
        "kv_slide_v": kv_slide_v,
    }
    payloads = {
...
class AsyncCacheWriter:
    def __init__(
        self, *, rank_dir: str, max_shard_bytes: int, max_queue_size: int = 128
    ):
        self.writer = LocalCacheWriter(
            rank_dir=rank_dir, max_shard_bytes=max_shard_bytes
        )
...
def load_local_summary(rank_dir: str):
    with open(os.path.join(rank_dir, "summary.json"), encoding="utf-8") as handle:
...
def finalize_index(*, output_dir, summaries, shard_map):
    ...
    with open(index_tmp_path, "wb") as output_handle:
        for summary in sorted(
            summaries, key=lambda item: int(item["source_sample_start"])
        ):
            rank_dir = os.path.join(
                output_dir, "_tmp", f"rank_{int(summary['global_rank'])}"
            )
            with open(
                os.path.join(rank_dir, "samples.local.idx"), "rb"
            ) as local_handle:
            ...
            for offset in range(0, len(local_bytes), INDEX_RECORD_SIZE):
                ...
                global_shard_id = shard_map[int(summary["global_rank"])][
                    int(record["shard_id"])
                ]
                output_handle.write(
                    pack_index_record(
...
def build_manifest(*, num_samples, shards, hidden_size, kv_dims, extra_fields=None):
    ...
    manifest = {
        ...
        "kv_dims": {
            name: [int(kv_dims[name][0]), int(kv_dims[name][1])] for name in _KV_FIELDS
        },
        "shards": shards,
    }
    if extra_fields:
...
def load_manifest(cache_dir: str):
    ...
    assert os.path.exists(manifest_path), (
        f"Missing target cache manifest: {manifest_path}"
    )
    ...
    for expected_id, shard in enumerate(manifest["shards"]):
        assert int(shard["shard_id"]) == expected_id, (
...
class CacheDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cache_dir: str, max_open_shards: int = 4):
        ...
        self.kv_dims = {
            name: (int(h), int(d)) for name, (h, d) in self.manifest["kv_dims"].items()
        }
    ...
    def _read_int(self, *, shard_mmap, offset, count, np_dtype, torch_dtype, nbytes):
        assert int(offset) + int(nbytes) <= shard_mmap.size(), (
            "tensor beyond shard size"
        )
        array = np.frombuffer(
            shard_mmap, dtype=np_dtype, count=int(count), offset=int(offset)
        ).copy()
    ...
    def _read_bf16(self, *, shard_mmap, offset, shape, nbytes):
        ...
        array = np.frombuffer(
            shard_mmap, dtype=np.uint16, count=int(np.prod(shape)), offset=int(offset)
        ).copy()
    ...
    def __getitem__(self, index: int):
        ...
        last_hidden = self._read_bf16(
            shard_mmap=shard_mmap,
            offset=off["last_hidden"],
            shape=(seq_len, self.hidden_size),
            nbytes=nbytes["last_hidden"],
        )
...
def collate_cache(batch, pad_token_id: int):
    ...
    kv = {
        k: torch.zeros((B, v[0], T, v[2]), dtype=batch[0][k].dtype)
