...
Run 8-GPU:
  torchrun --standalone --nproc_per_node 8 scripts/gemma4_mtp/prepare_cache.py \
      --target /path/to/gemma4/target \
      --data /path/to/train_regenerated.jsonl \
      --out-dir /path/to/mtp_cache/short_train \
...
def parse_args():
    ...
    ap.add_argument(
        "--out-dir", required=True, help="cache output dir (mount ok); must be empty"
    )
...
def main():
    ...
    from speculators.models.gemma4_mtp import target_cache as tc
    from speculators.models.gemma4_mtp.data import Gemma4ConversationParser, iter_jsonl
    ...
    summary = tc.LocalWriteSummary(
        ...
        kv_dims={
            name: [int(kv_dims[name][0]), int(kv_dims[name][1])]
            for name in tc._KV_FIELDS
        }
        if kv_dims
        else {},
    )
    ...
    if rank == 0:
        ...
        for r in range(world):
            ...
            while not os.path.exists(r_summary):
                ...
                if waited % 60 == 0:
                    print(
                        f"[rank0] waiting for rank {r} summary ({waited}s)...",
                        flush=True,
                    )
        ...
        kv_dims = {
            name: (int(h), int(d)) for name, (h, d) in dim_src["kv_dims"].items()
        }

        shard_map, shards = tc.build_global_shard_map(summaries)
        for summary_json in summaries:
            r_dir = os.path.join(
                args.out_dir, "_tmp", f"rank_{int(summary_json['global_rank'])}"
            )
            tc.rename_local_shards(
        ...
        print(
            f"[done] cache at {args.out_dir} "
            f"({num_samples} samples, {len(shards)} shards)",
            flush=True,
        )
