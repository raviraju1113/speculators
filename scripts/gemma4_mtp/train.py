#!/usr/bin/env python3
"""Fine-tune the Gemma 4 assistant (MTP draft) by TTT multi-step distillation.

Ties together the training-step interface
(speculators.models.gemma4_mtp.training_step) and the conversation data pipeline
(speculators.models.gemma4_mtp.data) into a runnable training loop.
...
Freeze policy:
  ...
  - assistant decoder layers + pre_projection + post_projection: TRAINED.

Run on a GPU box (needs GPU + both models):

    python scripts/gemma4_mtp/train.py \
        --target /path/to/gemma4/target \
        --assistant /path/to/gemma4/assistant \
        --data /path/to/train_regenerated.jsonl \
        --output ./out/gemma4_mtp \
        --epochs 1 --batch-size 2 --lr 1e-4 --bf16

Prints real per-step loss from the actual models; throughput/acceptance numbers
come from benchmarking the exported checkpoint in vLLM afterwards.
"""
...
def parse_args():
    ...
    ap.add_argument(
        "--data",
        default=None,
        help="target-regenerated conversations "
        "JSONL (required unless --cache-dir is set)",
    )
    ap.add_argument(
        "--cache-dir",
        default=None,
...
def set_trainable(target, assistant):
    ...
    print("[freeze] target: fully frozen", flush=True)
...
def main():
    ...
    if ddp:
        assistant = DDP(assistant, device_ids=[local_rank], find_unused_parameters=True)
    ...
    if use_cache:
        ...
        def collate_fn(b):
            return collate_cache(b, pad_token_id=pad_id)
    else:
        ...
        def collate_fn(b):
            return collate(b, pad_token_id=pad_id)
    ...
    log(
        f"=== Training: {len(dataset)} samples, {len(loader)} batches/epoch/rank, "
    ...
    def to_device(batch):
        ...
        for k, v in batch.items():
            if k == "shared_kv_states":
                out[k] = {
                    kt: (kv[0].to(device), kv[1].to(device)) for kt, kv in v.items()
                }
            else:
                out[k] = v.to(device)
        return out
